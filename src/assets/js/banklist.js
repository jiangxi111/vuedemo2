  ...
  $(function(){
	var pBankLength = $(".pBank ul li").length;
	var secLength = $(".sec ul li").length;
	if(pBankLength<=12){
		$(".bindNone").hide();
	}
    $(".fangsi label").on('click', function(event) {
        $(this).children("input").attr("checked",true).end().siblings().children("input").attr("checked",false);
        $(".bank li").find("input").attr("checked",false).end().find('i').removeClass("click");
        var indexLi = $(".fangsi label").index(this);
        $(".bangListDiv > div.bank").eq(indexLi).show().siblings().hide();
        if(indexLi==0){
        	if(pBankLength<=12){
        		$(".bindNone").hide();
        	}
    ...
    $(".subChongzhi").on('click', function() {
     ...
     for(i=0;i<nums;i++) {
      ...
      if(!flag){
       ...
       //获得单选按钮的值
      radioValue = $('.bank input:radio:checked').val();
      var isQuick = $('.fangsi input:radio:checked').val();
      $("#bankCode").val(radioValue);//银行
      $("#card").val(kahaoInput);//卡号
      $("#bandType").val(isQuick);//快捷还是普通
      $("#bindCardForm").submit();
   });

function ShowPre(o){
	...
	if(o.numIco){
		...
		for(var i = 0;i<numL;i++){
			numHtml+="<section class='numlist' attrNum="+i+"><span class=num"+i+"></span></section>";
		};
		this.numIco.html(numHtml);
		this.numIcoLi = this.numIco.find(".numlist");

		this.numIcoLi.bind("click",function(){
			//if(that.c==$(this).html())return false;
			if(that.c==$(this).attr("attrNum"))return false;
			that.c=$(this).attr("attrNum");
			that.move();
		});
	};
	this.bigBox = null;
